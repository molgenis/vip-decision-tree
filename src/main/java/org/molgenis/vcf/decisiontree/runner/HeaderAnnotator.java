...
public class HeaderAnnotator {
    ...
    if (settings.getMode() == Mode.VARIANT) {
      ...
      vcfHeader.addMetaDataLine(
          new VCFInfoHeaderLine(
              VIPC_S, VCFHeaderLineCount.UNBOUNDED, VCFHeaderLineType.String, VIPC_S_INFO));
      vcfHeader.addMetaDataLine(
  ...
  private static VCFHeader addVariantHeaders(
      VcfMetadata vcfMetadata,
      WriterSettings writerSettings,
      VCFHeader vcfHeader,
      VCFHeaderLine vepHeaderLine) {
  ...
  private static VCFHeaderLine createNewVepHeaderLine(
      WriterSettings writerSettings, VcfMetadata vcfMetadata) {
    ...
    return new VCFInfoHeaderLine(
        vepHeader.getID(),
        vepHeader.getCountType(),
        vepHeader.getType(),
        vepDescriptionBuilder.toString(),
        vepHeader.getSource(),
        vepHeader.getVersion());
  }
}
