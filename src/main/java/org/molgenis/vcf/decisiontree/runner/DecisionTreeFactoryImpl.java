...
class DecisionTreeFactoryImpl implements DecisionTreeFactory {
    ...
    private Node toNode(VcfMetadata vcfMetadata, String id, ConfigNode configNode,
        ...
        switch (configNode.getType()) {
            case EXISTS:
                ...
                break;
            case EXPRESSION:
                node = toExpressionNode(vcfMetadata, id, (ConfigExpressionNode) configNode, files);
                break;
            case BOOL:
    ...
    private Node toExpressionNode(VcfMetadata vcfMetadata, String id, ConfigExpressionNode configNode,
                                  Map<String, Set<String>> files) {
        Expression expression = toExpression(vcfMetadata, configNode.getExpression(), configNode.getFields(), files);
    ...
    private Expression toExpression(VcfMetadata vcfMetadata, ConfigExpression expression, String fieldsString, Map<String, Set<String>> files) {
        Map<String, Field> fields = new HashMap<>();
        for (String fieldMapping : fieldsString.split(",")) {
            String[] fieldPair = fieldMapping.split("=");
            Field field = vcfMetadata.getField(fieldPair[1]);
            fields.put(fieldPair[0], field);
        }
        Object value = expression.getValue();
        if (expression.getValue().toString().startsWith(FILE_PREFIX)) {
            value = files.get(((String) value).substring(FILE_PREFIX.length()));
        }
        Expression.Operator operator = toExpressionOperator(expression.getOperator());
        return Expression.builder().calculation(expression.getExpression()).value(Double.valueOf(value.toString())).fields(fields).operator(operator).build();
    }
    ...
    private Expression.Operator toExpressionOperator(ConfigOperator configOperator) {
        ...
        switch (configOperator) {
            case EQUALS:
                ...
                break;
            case NOT_EQUALS:
                operator = Expression.Operator.NOT_EQUALS;
                break;
            case LESS:
                operator = Expression.Operator.LESS;
                break;
            case LESS_OR_EQUAL:
                operator = Expression.Operator.LESS_OR_EQUAL;
                break;
            case GREATER:
                operator = Expression.Operator.GREATER;
                break;
            case GREATER_OR_EQUAL:
                operator = Expression.Operator.GREATER_OR_EQUAL;
                break;
            default:
                throw new UnexpectedEnumException(configOperator);
        }
        ...
        switch (configNode.getType()) {
            ...
            case EXPRESSION:
                updateExpressionNode(
                        (ExpressionNode) node, (ConfigExpressionNode) configNode, nodeMap, labelMap);
                break;
            default:
                throw new IllegalArgumentException(
                        String.format("unexpected enum '%s'", configNode.getType().toString()));
    ...
    private void updateExpressionNode(
            ExpressionNode node,
            ConfigExpressionNode configNode,
            Map<String, Node> nodeMap,
            Map<String, Label> labelMap) {
        NodeOutcome outcomeTrue = toNodeOutcome(configNode.getOutcomeTrue(), nodeMap, labelMap);
        node.setOutcomeTrue(outcomeTrue);
