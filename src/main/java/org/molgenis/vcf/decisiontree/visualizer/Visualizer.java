...
public class Visualizer {
  ...
  public static void main(String[] args) {
    ...
    Path outputPath =
        commandLine.hasOption(OPT_OUTPUT)
            ? Path.of(commandLine.getOptionValue(OPT_OUTPUT))
    ...
    for (Entry<String, ConfigNode> entry : tree.getNodes().entrySet()) {
      nodes.add(
          new Node(
              entry.getKey(), entry.getValue().getLabel(), entry.getValue().getType() == LEAF));
  ...
  private static Map<String, String> getBoolOutcomes(ConfigBoolMultiNode node) {
    ...
    for (ConfigBoolMultiQuery configBoolMultiQuery : node.getOutcomes()) {
      boolOutcomes.put(
          configBoolMultiQuery.getDescription(),
  ...
  private static Map<String, String> getCategoricalOutcomes(ConfigCategoricalNode node) {
    ...
    for (Entry<String, ConfigNodeOutcome> configEntry : node.getOutcomeMap().entrySet()) {
  ...
  private static void visualizeHtml(
      List<Node> nodes, List<Edge> edges, Path outputPath, String title, boolean isMermaidEnabled) {
    ...
    try {
      Path templatePath =
          Paths.get(
              Objects.requireNonNull(Visualizer.class.getResource("/template-mmd.html")).toURI());
      ...
      if (isMermaidEnabled) {
        Files.writeString(
            Path.of(outputPath.toString().replace(HTML, ".mmd")), mmdContent.toString());
      }
    } catch (IOException e) {
  ...
  private static void processOutcomes(
      List<Edge> edges, Entry<String, ConfigNode> entry, Map<String, String> outcomes) {
