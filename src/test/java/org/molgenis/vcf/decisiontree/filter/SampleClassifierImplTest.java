...
class SampleClassifierImplTest {
  @Mock private DecisionTreeExecutor decisionTreeExecutor;
  @Mock private VcfMetadata vcfMetadata;
  @Mock private VcfReader vcfReader;
  @Mock private DecisionTree decisionTree;
  @Mock private RecordWriter recordWriter;
  @Mock private VepHelper vepHelper;
  @Mock private SampleAnnotator sampleAnnotator;

  private Classifier classifier;
  private FieldImpl parent;
  private NestedHeaderLine nestedHeaderLine;
  private SampleContext sampleContext1 =
      SampleContext.builder()
          ...
          .phenotypes(List.of())
          .build();
  private SampleContext sampleContext2 =
      SampleContext.builder()
          .index(1)
          .proband(true)
          .sex(Sex.MALE)
          ...
          .phenotypes(List.of())
          .build();
  private SamplesContext samplesContext =
      SamplesContext.builder().sampleContexts(Set.of(sampleContext1, sampleContext2)).build();
  ...
  void setUp() {
    ...
    nestedHeaderLine =
        NestedHeaderLine.builder().parentField(parent).nestedFields(nestedFields).build();
    when(vcfMetadata.getVepHeaderLine()).thenReturn(nestedHeaderLine);
    classifier =
        new SampleClassifierImpl(
  ...
  void classify() {
    ...
    org.molgenis.vcf.decisiontree.filter.Allele allele0_1 =
        org.molgenis.vcf.decisiontree.filter.Allele.builder().bases("G").index(0).build();
    ...
    org.molgenis.vcf.decisiontree.filter.Allele allele1_1 =
        org.molgenis.vcf.decisiontree.filter.Allele.builder().bases("G").index(0).build();
    org.molgenis.vcf.decisiontree.filter.Allele allele1_2 =
        org.molgenis.vcf.decisiontree.filter.Allele.builder().bases("T").index(1).build();
    ...
    Map<Integer, List<VcfRecord>> recordMap1 = Map.of(1, List.of(record1a), 2, List.of(record1b));
    when(vepHelper.getRecordPerConsequence(record0, nestedHeaderLine)).thenReturn(recordMap0);
    when(vepHelper.getRecordPerConsequence(record1, nestedHeaderLine)).thenReturn(recordMap1);
    Decision decision1a =
        Decision.builder().clazz("test1a").path(List.of()).labels(Set.of()).build();
    Decision decision2a =
        Decision.builder().clazz("test2a").path(List.of()).labels(Set.of()).build();
    Decision decision2b =
        Decision.builder().clazz("test2b").path(List.of()).labels(Set.of()).build();

    doReturn(decision1a)
        .when(decisionTreeExecutor)
        .execute(decisionTree, new Variant(vcfMetadata, record0a, allele0_1), sampleContext1);
    doReturn(decision2a)
        .when(decisionTreeExecutor)
        .execute(decisionTree, new Variant(vcfMetadata, record1a, allele1_1), sampleContext1);
    doReturn(decision2b)
        .when(decisionTreeExecutor)
        .execute(decisionTree, new Variant(vcfMetadata, record1b, allele1_2), sampleContext1);
    doReturn(decision1a)
        .when(decisionTreeExecutor)
        .execute(decisionTree, new Variant(vcfMetadata, record0a, allele0_1), sampleContext2);
    doReturn(decision2a)
        .when(decisionTreeExecutor)
        .execute(decisionTree, new Variant(vcfMetadata, record1a, allele1_1), sampleContext2);
    doReturn(decision2b)
        .when(decisionTreeExecutor)
        .execute(decisionTree, new Variant(vcfMetadata, record1b, allele1_2), sampleContext2);

    classifier.classify(vcfReader);

    verify(sampleAnnotator)
        .annotate(eq(List.of(new Decision("test1a", List.of(), Set.of()))), eq(0), any());
    verify(sampleAnnotator)
        .annotate(eq(List.of(new Decision("test1a", List.of(), Set.of()))), eq(1), any());
    verify(sampleAnnotator)
        .annotate(
            eq(
                List.of(
                    new Decision("test2a", List.of(), Set.of()),
                    new Decision("test2b", List.of(), Set.of()))),
            eq(0),
            any());
    verify(sampleAnnotator)
        .annotate(
