...
class ConsequenceAnnotatorImplTest {
  ...
  @Mock private VepMetadataMapper vepMetadataMapper;
  ...
  void annotateLabelsPath() {
    ...
    when(vepMetadataMapper.map("CSQ", vcfHeader))
        .thenReturn(
            NestedHeaderLine.builder()
                .nestedFields(
                    Map.of(
                        "VIPC",
                        new NestedField(
                        ...
                        new NestedField(
                            ...
                            5,
                            vepfield)))
                .parentField(vepfield)
                .build());

    List<Node> path = List.of(node1, node2, node3);
    Decision decision = Decision.builder().clazz(clazz).labels(labels).path(path).build();

    ConsequenceAnnotator annotator =
        new ConsequenceAnnotatorImpl(true, true, vcfHeader, vepMetadataMapper);
    assertEquals(
        "test1||test3|EXIT_NOW|node1&node2&node3|id1&id2",
        annotator.annotate(decision, "test1||test3"));
  }
  ...
  void annotateLabels() {
    ...
    ConsequenceAnnotator annotator =
        new ConsequenceAnnotatorImpl(true, false, vcfHeader, vepMetadataMapper);
  ...
  void annotate() {
    ...
    Decision decision = Decision.builder().clazz(clazz).labels(labels).path(emptyList()).build();
    ConsequenceAnnotator annotator =
        new ConsequenceAnnotatorImpl(false, false, vcfHeader, vepMetadataMapper);
    assertEquals(
        "test1|EXIT_NOW|test3", annotator.annotate(decision, "test1|PREVIOUS_ANNOTATION|test3"));
  }
}
